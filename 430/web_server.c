 ...
 * 硬件连接： ------------------------------------
 *           |            ：ENC28J60-INT (没用到)|
 *           |P6-4SPI_MISO：ENC28J60-SO          |
 *           |P6-5SPI_MOSI：ENC28J60-SI          |
 *           |P6-6SPI_SCK ：ENC28J60-SCK         |
 *           |P6-7SPI_CS  ：ENC28J60-CS          |
 *           |            ：ENC28J60-RST (没用)  |
 *            ------------------------------------
********************************************************/
#include "enc28j60.h"
...
unsigned char analyse_get_url(char *str)
{
    ...
    if (*str < 0x3a && *str > 0x2f)   //asii 58> str >47 
    {
        // is a ASCII number, return it
        return(*str-0x30); //change to num
    }
    return(-2);
}
...
unsigned int print_webpage(unsigned char *buf,unsigned char on_off)
{
    ...
    plen=fill_tcp_data_p(buf,plen,PSTR("<title> Web Server! </title>")); //网页标签
	
		/* 在网页正中央填写信息 */
    
    /* plen=fill_tcp_data_p(buf,plen,PSTR("<center><h1> 基于MSP430单片机的智能家居灯光控制系统</h1>"));
    plen=fill_tcp_data_p(buf,plen,PSTR("<p> LED状态指示: "));
    ...
    if (on_off)
    {
       plen=fill_tcp_data_p(buf,plen,PSTR("/2\">关闭LED</a><p>"));
    }
    else
    {
        plen=fill_tcp_data_p(buf,plen,PSTR("/3\">点亮LED</a><p>"));
    }
    plen=fill_tcp_data_p(buf,plen,PSTR("<hr><br>****************^_^ 欢迎您进入本界面 ^_^****************\n</center>"));  */
 ...
 * 输出  ：无
 * 返回  ：-0 运行成功
 * 应用  ：1 在PC机的DOS界面输入： ping 192.168.0.15 (看能否ping通)
 *         2 在IE浏览器中输入：http://192.168.0.15/123456 则会出现一个网页，通过网页
 *           中的命令可以控制开发板中的LED的亮灭
 */
int Web_Server(void)
{   
...
/*-----------------tcp port www start, compare only the lower byte-----------------------------------*/
      if (buf[IP_PROTO_P]==IP_PROTO_TCP_V&&buf[TCP_DST_PORT_H_P]==0&&buf[TCP_DST_PORT_L_P]==mywwwport)
      {
          ...
          if (buf[TCP_FLAGS_P] & TCP_FLAGS_ACK_V)
          {
              ...
              cmd=analyse_get_url((char *)&(buf[dat_p+5]));  //takes a string of the form password/commandNumber and analyse it
              // for possible status codes see:
              // http://www.w3.org/Protocols/rfc2616/rfc2616-sec10.html
              if (cmd==-1)
              {
